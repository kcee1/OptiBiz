...
namespace DataAccessLayer.Repositories
{
    public class BaseRepository<T> : IDisposable, IRepository<T> where T : class
    {
        ...
        public virtual IEnumerable<T> GetBy(Expression<Func<T, bool>> predicate, Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null, int? skip = null, int? take = null, params string[] includeProperties)
        {
            try
            {
        ...
        /// <param name="take">maximum number of objects to be returned</param>
        /// <param name="includeProperties">the properties to be included in query</param>
        public virtual async Task<IEnumerable<T>?> GetByAsync(Expression<Func<T, bool>> predicate, Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null, int? skip = null, int? take = null, params string[] includeProperties)
        {
            try
            {
                IQueryable<T> query = ConstructQuery(predicate, orderBy, skip, take, includeProperties);
        ...
        /// <summary>
        /// Returns a paginated result for a request
        /// </summary>
        /// <param name="pagination">Indicates the parameters for the pagination</param>
        /// <param name="filter">The paramters for the request condition</param>
        /// <param name="orderBy">Ordering system where application</param>
        /// <param name="includeProperties">Navigation properties to include <see langword="in"/>request where applicable</param>
        /// <returns>Paginated response to the request</returns>
        /// <example>
        ///     <code>
        ///             var vouchers = await _voucherRepository.GetPaginated(
        ///                      filter: a => a.UserId == userid && a.TimeUsed == null,
        ///                    pagination: new PaginationQueryParameters
        ///                   {
        ///                       PageNo = pageNo > 0 ? pageNo : 1,
        ///                       PageSize = pgSize > 0 && pgSize< 100 ? pgSize : 100
        ///                   },
        ///                   orderBy: a => a.OrderByDescending(keySelector: a =>                                      a.TimeAdded
        ///                 )
        ///             );
        ///     </code>
        /// </example>
        public async Task<PaginationResult<T>> GetPaginated(PaginationQueryParameters pagination, Expression<Func<T, bool>>? filter = null, Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null, params string[] includeProperties)
        {
            ...
            query = query.Skip((pagination.PageNo - 1) * pagination.PageSize).Take(pagination.PageSize).AsNoTracking();
